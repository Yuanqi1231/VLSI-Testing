...
\begin{frame}{问题定义：同一条链路，两种观测体制}
    ...
    \begin{tikzpicture}[
        ...
        \node[box,fill=green!12,anchor=west] (full) at ($(wave.east)+(1.0,1.05)$)
            {\textbf{完整波形}（bring-up / 后硅）\\1280 点全部可见};
        \node[box,fill=red!12,anchor=west] (lossy) at ($(wave.east)+(1.0,-1.05)$)
            {\textbf{有损标量}（运行时）\\单个眼裕度数字};
        \node[box,fill=yellow!20,anchor=north west] (tel) at ($(sim.south west)+(0,-1.45)$)
